...
def test_iter_chunk_results_falls_back_to_sequential_after_pool_failure(monkeypatch) -> None:
    class FakeFuture:
        def __init__(self, value=None, exc: Exception | None = None, order: int = 0) -> None:
            self._value = value
            self._exc = exc
            self.order = order
    ...
    class FakePool:
        ...
        def submit(self, func, args):
            order = self.submissions
            self.submissions += 1
            if args[0] == "b":
                return FakeFuture(exc=RuntimeError("pool failure"), order=order)
            return FakeFuture(value=func(args), order=order)
...
def test_incremental_index_rolls_back_file_update_when_state_save_fails(monkeypatch, tmp_path: Path) -> None:
    config = _config(tmp_path)
    store = _FakeStore()
    stats = IndexStats(1, 1, 10, 10, "2026-01-01T00:00:00+00:00", [], "simple-hash", "project_docs")
    ...
    monkeypatch.setattr(
        store,
        "get",
        lambda ids=None, include=None, where=None: {
            "ids": ["old-a1"],
            "documents": ["old doc"],
            "metadatas": [{"source_file": "docs/a.md"}],
            "embeddings": [[0.1, 0.2]],
        },
    )
    monkeypatch.setattr(
        store,
        "upsert",
        lambda ids, documents, embeddings, metadatas: events.append(("restore", ids)),
    )
    monkeypatch.setattr("gy_doc_search.indexer._save_state", lambda cfg, current_state: (_ for _ in ()).throw(RuntimeError("save failed")))
    monkeypatch.setattr("gy_doc_search.indexer._build_stats", lambda cfg, current_store, current_state: stats)

    try:
        incremental_index(config)
        assert False, "incremental_index should have raised"
    ...
    assert state["files"]["docs/a.md"]["chunk_ids"] == ["old-a1"]
